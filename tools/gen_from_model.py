...
from chat.prompts import render as render_prompt
...
class ModelAdapter:
    ...
    def propose_config(
        ...
        if self._chat_client is not None:
            ...
            system_msg = render_prompt("propose_config", example_json=json.dumps(example))
    ...
    def evaluate_and_plan(
        ...
        if self._chat_client is not None:
            ...
            text = self._chat_client.chat(
                messages=[system_msg, {"role": "user", "content": json.dumps(payload)}],
                temperature=temperature,
                max_tokens=800,
            )
